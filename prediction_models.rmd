...
In the tables, 20 features with top effects are presented for two models:
...
kable(rowMeans(pc$betas[-c(1:3),])[names(head(sort(abs(rowMeans(pc$betas[-c(1:3),])),decreasing = T),n = 20))])
predictor_names = names(head(sort(abs(rowMeans(p$betas[-c(1:3),])),decreasing = T),n = 20))
predictor_names2 = names(head(sort(abs(rowMeans(pc$betas[-c(1:3),])),decreasing = T),n = 20))
```
...
samples = 1:nrow(all_taxa_log)
folds = createFolds(samples,k = 10)
result = data.frame(features = predictor_names2,aucz = rep(NA,20),aucTrain = NA,aucTest= NA)
for(j in 1:20){
...
### AUC plots for 10-fold CV

Smoothed curves for 10 folds are plotted. 
...
legend("bottomright",c("MB","Calprot","Base"),col = c("red","skyblue4","black"),lty=c(1,1,1),lwd = 2 )
...
```{r}
#calprotectin agaist intrinsic predictors
#wilcox.test(cv_t_result$auc_zero,cv_calprot_result$auc,alternative = "greater",paired = T)
#microbiome+calprotectin against only calprotectindata.frame(zero_pred = cv_t_result$auc_zero,
...
#wilcox.test(cv_calprot_result$auc,calprot = cv_tc_result$auc,alternative = "greater")
```
